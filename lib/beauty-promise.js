...
function queryUnderstanding(params) {
    ...
    tick.start();
    conditions = {
        tag: utils.extractTag(params.text) || '正妹',
...
function applyPremiumPlans(params) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        if(premiumPlans && premiumPlans.length) {
            ...
            if (matchedPlan && !matchedPlan.conditions) {
                params.err = new Error('premium plan should contain the key conditions');
                tick.end();
                reject(params);
            } else if (matchedPlan && matchedPlan.conditions) {
...
function conditionalSearch(db) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        var _query = _merge({}, search.query.base, search.query.conditions);
        debug('query:', _query);
        coLists.find(_query)
        .sort(search.order)
        .limit(1)
